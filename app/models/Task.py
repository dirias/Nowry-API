...
class Task(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Additional details about the task"
    )
    ...
    deadline: Optional[datetime] = Field(
        None, description="When the task should be completed"
    )
    created_at: datetime = Field(..., description="Timestamp of task creation")
    updated_at: datetime = Field(..., description="Timestamp of last update")
    tags: List[str] = Field(
        default_factory=list, description="Labels or categories for filtering/sorting"
    )
    position: int = Field(
        ..., description="Order of task in list (for manual drag & drop sorting)"
    )
    repeat_interval: Optional[str] = Field(
        None, description="Repetition setting like 'daily', 'weekly', etc."
    )
